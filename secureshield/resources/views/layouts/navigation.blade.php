<nav x-data="{ open: false }" class="bg-gradient-to-r from-blue-600 to-blue-800 shadow-lg">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    <x-nav-link :href="route('user.subscription.index')" :active="request()->routeIs('user.subscription.*')">
                        {{ __('Mi Suscripción') }}
                    </x-nav-link>
                    @endif
                </div>
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6">
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        <button class="inline-flex items-center px-4 py-2 border border-blue-400 text-sm leading-4 font-medium rounded-lg text-white bg-blue-700 hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-blue-800 transition ease-in-out duration-150">
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden bg-blue-700">
        <div class="pt-2 pb-3 space-y-1">
            ...
            @if(!Auth::user()->isAdmin())
            <x-responsive-nav-link :href="route('user.subscription.index')" :active="request()->routeIs('user.subscription.*')">
                {{ __('Mi Suscripción') }}
            </x-responsive-nav-link>
            @endif
        </div>
